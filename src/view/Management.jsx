...
const Management = () => {
  ...
  useEffect(() => {
    ...
    const fetchDishes = async () => {
      try {
        const response = await fetch(
          'https://10.120.32.81/restaurant/api/v1/dishes'
        );
    ...
    const fetchAllergens = async () => {
      try {
        const response = await fetch(
          'https://10.120.32.81/restaurant/api/v1/allergens'
        );
  ...
  const deleteMenu = async (id) => {
    const confirmDelete = window.confirm(
      'Haluatko varmasti poistaa tämän menun?'
    );
  ...
  const deleteDish = async (id) => {
    const confirmDelete = window.confirm(
      'Haluatko varmasti poistaa tämän ruokalajin?'
    );
    if (!confirmDelete) return;

    try {
      const response = await fetch(
        `https://10.120.32.81/restaurant/api/v1/dishes/${id}`,
        {
          method: 'DELETE',
        }
      );
  ...
  const handleUpdateMenu = async (e) => {
    ...
    try {
      const response = await fetch(
        `https://10.120.32.81/restaurant/api/v1/menus/${editingMenu.id}`,
        {
          method: 'PUT',
          body: formData,
        }
  ...
  return (
    <div className="management-container">
      <button
        ...
        className="go-to-reservation-btn"
      >
        Siirry varauksiin
      </button>
            ...
            <form
              onSubmit={async (e) => {
                ...
                const url = editingDish
                  ? `https://10.120.32.81/restaurant/api/v1/dishes/${editingDish.id}`
                  : `https://10.120.32.81/restaurant/api/v1/dishes`;
          ...
          <div id="menu-modal-content">
            ...
            <form
              onSubmit={async (e) => {
                ...
                const url = `https://10.120.32.81/restaurant/api/v1/allergens`;
                const method = 'POST';
      ...
      {showLinkModal && (
        <div className="menu-modal">
          <div id="menu-modal-content">
            ...
            <h5>
              {linkActionType === 'link'
                ? 'Linkitä allergeeni ruokalajiin'
                : 'Irroita allergeeni ruokalajista'}
            </h5>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                const endpoint =
                  linkActionType === 'link'
                    ? 'https://10.120.32.81/restaurant/api/v1/allergens/link'
